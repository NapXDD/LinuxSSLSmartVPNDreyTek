...
E2E_ENV_VARS=(DRAYTEK_GATEWAY DRAYTEK_USERNAME DRAYTEK_PASSWORD
              DRAYTEK_PORT DRAYTEK_VERIFY_CERT DRAYTEK_DEFAULT_ROUTE PING_TARGET)
...
run "mkdir -p /run/dbus && (dbus-daemon --system --fork || true) && /usr/sbin/NetworkManager && sleep 2"
...
run "nmcli connection add type vpn ifname '*' con-name smoke \
    vpn-type draytek \
    vpn.data 'gateway=${DRAYTEK_GATEWAY:-vpn.invalid},port=${DRAYTEK_PORT:-443},username=${DRAYTEK_USERNAME:-smoke},verify-cert=no,never-default=${NEVER_DEFAULT}'"
